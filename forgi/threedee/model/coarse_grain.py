...
class 2Dprojection(object):
    """A 2D Projection of a CoarseGrainRNA unto a plain"""
    def __init__(self, cg, proj_direction):
        ...
        for key, val in cg.coords.values():
    ...
    @property
    def proj_plane(self):
        """A tuple of unit vectors creating the projection plane"""
        return self._unit_vec1, self._unit_vec2
    @property
    def crossingPoints(self):
        ...
        if self._cross_points is None:
            self._cross_points=c.defaultdict(list)
    ...
    def build_proj_graph(self):
        proj_graph=c.defaultdict(set)            
        for key, element in self._coords.items():
            crs=self.crossingPoints
            sortedCrs=ftuv.sortAlongLine(element[0], element[1], crs[key]) #TODO implement.
            oldpoint=None
    ...
    def proj_graph(self):
        if self._proj_graph is None:
            self.build_proj_graph()
        return self._proj_graph
    @property
    def convex_hull(self):
        points=[ x for x in a for a in self.coords.values()]
