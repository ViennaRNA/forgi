...
def next_residue(draw, last_residue, with_missing=False, with_insertion=False, with_chainbreak=False):
    ...
    if with_chainbreak:
        # Don't sample chain-breaks without chain
        assume(last_residue[0] is not None)
...
def _seqids_strategy_core(draw, with_missing, with_insertions, with_chainbreak):
    first_res = draw(resid_strategy(
        i_kwargs={"min_value": -25, "max_value": 200}))
    if with_chainbreak:
        assume(first_res[0] is not None)
    length = draw(integers(min_value=with_missing +
                           with_insertions + with_chainbreak + 1, max_value=100))
    ...
    for i, s in enumerate(seqids):
        if i > 0:
            ...
            if s[1][2] != " ":
                ...
                if seqids[i - 1][0] == s[0]:
                    assert seqids[i -
                                  1][1][1] == s[1][1], "{}, {}".format(seqids[i - 1], s)
            if s[0] != seqids[i - 1][0]:
                found_c = True
    assert found_c
    assert found_i
