...
class Projection2DBasicTest(unittest.TestCase):
    ...
    def test_init_with_rotate_for_virtual_atoms(self):
        ...
        projRot = fpp.Projection2D(
            cg, [1., 1., 1.], rotation=90, project_virtual_atoms=True)
    ...
    def test_init_with_rotate_for_virtual_residues(self):
        cg = ftmc.CoarseGrainRNA.from_bg_file(
            'test/forgi/threedee/data/1y26.cg')
        proj = fpp.Projection2D(
            cg, [1., 1., 1.], project_virtual_residues=[1, 30])
        projRot = fpp.Projection2D(
            cg, [1., 1., 1.], rotation=90, project_virtual_residues=[1, 30])
    ...
    def test_projection_with_some_virtual_atoms(self):
        cg = ftmc.CoarseGrainRNA.from_bg_file(
            'test/forgi/threedee/data/1y26.cg')
        proj = fpp.Projection2D(
            cg, [1., 1., 1.], project_virtual_atoms="selected")
        proj_all = fpp.Projection2D(
            cg, [1., 1., 1.], project_virtual_atoms=True)
        all_vas = sum(len(cg.virtual_atoms(i).keys()) for d in cg.defines.keys()
                      for i in cg.define_residue_num_iterator(d))
        self.assertLess(proj._virtual_atoms.shape[0], all_vas)
        self.assertLess(
            proj._virtual_atoms.shape[0], proj_all._virtual_atoms.shape[0])
        # At least some atoms
        self.assertGreater(proj._virtual_atoms.shape[0], 5)
    ...
    def test_projection_with_nonexisting_virtual_residues(self):
        ...
        with self.assertRaises(LookupError):
            proj = fpp.Projection2D(
                cg, [1., 1., 1.], project_virtual_residues=[len(cg.seq) + 2])
        with self.assertRaises(LookupError):
            proj = fpp.Projection2D(
                cg, [1., 1., 1.], project_virtual_residues=[0])

    def test_get_vres_by_position(self):
        cg = ftmc.CoarseGrainRNA.from_bg_file(
            'test/forgi/threedee/data/1y26.cg')
        proj = fpp.Projection2D(
            cg, [1., 1., 1.], project_virtual_residues=[1, 4, 23])
    ...
    def test_vres_iterator(self):
        cg = ftmc.CoarseGrainRNA.from_bg_file(
            'test/forgi/threedee/data/1y26.cg')
        proj = fpp.Projection2D(
            cg, [1., 1., 1.], project_virtual_residues=[3, 7, 26])
        self.assertEqual(sorted(list(x[0]
                                     for x in proj.vres_iterator)), [3, 7, 26])
...
class Projection2DTestWithData(unittest.TestCase):
    def setUp(self):
        cg, = ftmc.CoarseGrainRNA.from_pdb('test/forgi/threedee/data/1y26_two_chains.pdb',
                                           dissolve_length_one_stems=False, load_chains="X")
    ...
    def test_condense(self):
        ...
        self.assertAlmostEqual(
            self.proj2.get_longest_arm_length()[0], 35.9, places=1)
        # self.proj.plot(show=True)
        self.proj2.condense(5)
        # self.proj2.plot(show=True)
        self.assertEqual(self.proj2.get_cyclebasis_len(), 2)
        self.assertAlmostEqual(self.proj2.get_longest_arm_length()[
                               0], 20.393064351368327)


@unittest.skip("TODO. Changes code for PDB-loading probably changed made projection direction ")
class Projection2DTestOnCondensedProjection(unittest.TestCase):
    def setUp(self):
        cg, = ftmc.CoarseGrainRNA.from_pdb('test/forgi/threedee/data/1y26_two_chains.pdb', dissolve_length_one_stems=False,
                                           load_chains="X")
        self.proj = fpp.Projection2D(cg, [1., 1., 1.])
    ...
    def test_get_maximal_path_length(self):
        self.assertAlmostEqual(
            self.proj.get_maximal_path_length(), 95.4, places=1)

    def test_get_longest_arm_length(self):
        self.assertAlmostEqual(self.proj.get_longest_arm_length()[
                               0], 20.123454285158793)
...
class Projection2DImageHelperfunctionTests(unittest.TestCase):
    ...
    def test_to_grayscale(self):
        np.testing.assert_almost_equal(fpp.to_grayscale(
            self.img_rgb), self.img_bw, decimal=2)
    ...
    def test_grayscale_to_grayscale(self):
        np.testing.assert_array_equal(
            fpp.to_grayscale(self.img_bw), self.img_bw)
