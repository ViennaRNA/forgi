...
class TestHausdorffDistances(unittest.TestCase):
    ...
    def test_modified_hausdorff_distance(self):
        ...
        self.assertEqual(
            fph.modified_hausdorff_distance(self.img, self.img), 0)
        self.assertEqual(fph.modified_hausdorff_distance(
...
class TestDistanceCgToImg(unittest.TestCase):
    def setUp(self):
        ...
        self.ref_proj = fpp.Projection2D(
            self.cg, [1., 1.,   1.], project_virtual_atoms=True)
        self.ref_proj_na = fpp.Projection2D(
            self.cg, [1., 1.,   1.], project_virtual_atoms=False)
    ...
    def test_try_parameters(self):
        ref_box = self.ref_proj.get_bounding_square(margin=30)
        ref_img, _ = self.ref_proj.rasterize(
            70, bounding_square=ref_box, rotate=45)
        scale = ref_box[1] - ref_box[0]
        distance, img, params = fph.try_parameters(ref_img, scale, self.cg, rotations=[
                                                   0, 30, 60, 90, 180, 270], proj_directions=[list(fph.to_polar([1, 1.1, 1.2]))[1:]])
        self.assertLessEqual(distance, 2)
    ...
    def test_global_search(self):
        ...
        ref_img, _ = self.ref_proj_na.rasterize(
            70, bounding_square=ref_box, rotate=0)
        scale = ref_box[1] - ref_box[0]
        distance, img, params = fph.globally_minimal_distance(
            ref_img, scale, self.cg, virtual_atoms=False, verbose=True)
        ...
        nptest.assert_allclose(
            params[0], fph.to_polar([2, 0, -1.2])[1:], atol=5)
