...
class TestJustClassifyFunctions(unittest.TestCase):
    def setUp(self):
        self.cg = ftmc.CoarseGrainRNA.from_bg_file(
            "test/forgi/threedee/data/3pdr_X.cg")

    def test_all_interactions(self):
        interactions = ftca.all_interactions(self.cg)
        print(interactions)
        self.assertGreaterEqual(len(interactions), 4)
        # We have 23 annotated Fr3d interactions, but some may be directly connected.
        self.assertLessEqual(len(interactions), 25)
        ...
        self.assertEqual(len(interactions), len(
            set([e[0] for e in interactions])))  # Loops are unique
...
class TestClassifier(unittest.TestCase):
    def test_via_crossval_slowtest(self):
        ...
        for i, s in enumerate(specs):
            assert s > 0.5, "Specificity({})={} <0.7".format(
                loops[i], specificity)
        for i, s in enumerate(sens):
            assert sensitivity > 0.5, "Sensitivity({})={} <0.7".format(
                loops[i], sensitivity)
...
class TestClassifyCoreImplementation(unittest.TestCase):
    def test_no_interactions(self):
        ...
        self.assertEqual(
            ftca._classify_potential_interactions(clf, [], []), [])
