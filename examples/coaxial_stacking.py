...
try:
    ...
    if pymbar.version.version.split(".")[0] != '3':
        log.warning(
            "Wrong version of optional dependency pymbar: Found {}, need 3+.".format(pymbar.version.version))
        pymbar = None

except ImportError as e:
    log.info(
        "Optional dependency pymbar could not be imported. Reason: '{}'".format(e))
    pymbar = None
...
def plot_kde(data, ax, settings):
    ...
    if settings["bandwidth"] is not None:
        ...
        grid = GridSearchCV(KernelDensity(kernel=settings.get("kernel", "tophat")),
                            {'bandwidth': np.linspace(
                                math.radians(2), math.radians(30), 40)},
    ...
    ax2 = polar_twin(ax)
    ax2.plot(x, np.exp(kde.score_samples(x.reshape(-1, 1))), label="kde",
             linewidth=settings.get("kde_linewidth", 2), color=settings.get("kde_color", "red"))
...
def show_circlehist(data, title, subplots, settings):
    ...
    if len(data):
        if num_bins is not None:
            ...
            bars = mainAx.bar(bins[:-1], values, width=2 *
                              math.pi / num_bins, linewidth=0.25)
        maxc = max(values)
        if max_val:
        ...
        if subplots:
            for ang_type, ax in [(2, ax[0, 1]), (3, ax[1, 0]), (4, ax[1, 1])]:
                data_f = data[(data.angle_type == ang_type) |
                              (data.angle_type == -ang_type)]
                if num_bins is not None:
                    values, bins = bin_angular(data_f, num_bins)
                    bars = ax.bar(bins[:-1], values, width=2 *
                                  math.pi / num_bins, linewidth=0.25)
                for r, bar in zip(values, bars):
                    bar.set_facecolor(plt.cm.jet(r / maxc))
...
def show(data):
    for a_type in [2, 3, 4]:
        try:
            st = pd.value_counts(data[(data.angle_type == a_type) | (
                data.angle_type == -a_type)]["is_stacking_dssr"])
        except KeyError:
            t = 0
            f = len(data[(data.angle_type == a_type)
                         | (data.angle_type == -a_type)])
        else:
            try:
                t = st[True]
            except:
        ...
        try:
            ...
            r = float("nan")
        ang = av_angle(data[(data.angle_type == a_type) |
                            (data.angle_type == -a_type)]["angle"])
...
def distribution_change(data, key, target_range):
    ...
    for i in target_range:
        ...
        for j in range(4):
            k = "{:d}° - {:d}°".format(
                int(math.degrees(bins[j])), int(math.degrees(bins[j + 1])))
            if first:
                keys.append(k)
...
def subsample_pymbar(data):
    if pymbar:
        ...
        indices = pymbar.timeseries.subsampleCorrelatedData(
            sorted_data.angle, g, conservative=True)
        subsampled = sorted_data.iloc[indices]
        subsampled.history = sorted_data.history + \
            ["subsampled ({})".format(math.ceil(g))]
    ...
    def __setitem__(self, key, val):
        ...
        if (val == "None" or val is None) and self.none_allowed[key]:
            ...
            if self.restricted[key] and val not in self.restricted[key]:
                raise ValueError("Value for configuration variable {} must be one of {}".format(
...
class interactive_analysis(object):
    ...
    def start(self):
        try:
            ...
            try:
                ...
                notification = Notify.Notification.new(
                    "Loading of Data complete.", "Interactive coaxial stacking analysis is ready")
                notification.show()
            except:
                pass
        ...
        while True:
            try:
                ...
                print(ENDC)
                # input was imported from future
                command = input("Please enter a command (HELP to show help): ")
                ...
                for comm in commands:
                    try:
                        ...
                        print(FAIL)
                        traceback.print_exc()
                        print("An exception of type {} occurred: {}".format(
                            type(e), e), ENDC)
                        # raise
    ...
    def _apply_filter(self, *args):
        ...
        try:
            ...
            raise InvalidInput(
                "Cannot convert value {} to type {} of column {}.".format(value, d_type, key))
    ...
    def _filter_from_r(self, key, r):
        ...
        if isinstance(r, int):
            ...
            data = self.ops["<"](self.ops[">="](
                self.filtered_data, key, r[0]), key, r[1])
        return data
    ...
    # In python3: (self, *args, one=False)
    def show_circularKDE(self, *args, **kwargs):
        """
        ...
        if args and args[0] == "for":
            ...
            raise InvalidInput(
                "Use the commands S and S1 with no arguments or with 'for' as the first argument. Found {}".format(repr(args)))
        else:
            title = hist_to_title(self.filtered_data.history)
    ...
    def plot_dc(self, *args):
        ...
        if args[:1] == ["for", "saved"]:
            raise InvalidInput(
                "The PLOT_DC command does not support the 'for saved' syntax.")
        if args[0] == "for":  # for is optional
            args = args[1:]
        key = args[0]
        if key not in self.filtered_data.columns.values:
            raise InvalidInput(
                "Invalid key '{}' used in 'for key' syntax".format(key))
    ...
    def save(self, *args):
        ...
        if len(args) > 1 and args[1] == "for":
            ...
            if args[0] == "for":
                raise InvalidInput(
                    "Name 'for' is not allowed as a name for saving data, because it is a keyword")
    ...
    def del_saved(self, *args):
        ...
        if len(args) == 0:
            raise InvalidInput(
                "Need at least one name of a save which should be deleted.")
        for name in args:
    ...
    def print_data(self, *args):
        ...
        if len(args) != 1 or args[0] not in self.filtered_data.columns.values:
            raise InvalidInput(
                "Expecting one argument containing a valid key!")
        else:
    ...
    def import_dataset(self, *args):
        ...
        if len(args) != 2:
            raise InvalidInput(
                "Expecting 2 arguments: path and name. Found {}".format(args))
        data = pd.read_csv(os.path.expanduser(args[0]))
        if args[1] in self.data:
            raise InvalidInput(
                "Name {} exists already. Please choose another name.")
        data.history = []
        data.full_dataset_name = args[1]
        self.data[args[1]] = data
        print (
            "Dataset has been imported to {}. Use SELECT_DATASET to select it.".format(args[1]))
    ...
    def _save_by_seed(self, data, prefix=""):
        ...
        for seed in range(1, 10):
            ds = self.ops["contains"](
                data, "parent_dir", "seed{}".format(seed))
            if len(ds):
    ...
    def workflow(self, *args):
        ...
        if args[0].upper() == "Bin_By_Seed".upper():
            ...
            STEPS = [62, 93, 140, 210, 315, 473, 710, 1065, 1598,
                     2397, 3596, 5394, 8091, 12137, 18206, 27309, 40964]
            ...
            for run in runs:
                ...
                run_data = self.ops["contains"](
                    self.filtered_data, "parent_dir", run)
                plot_p = []
                plot_step = []
                for max_steps in STEPS:
                    ...
                    for seed in range(1, 10):
                        ds = self.ops["contains"](
                            step_data, "parent_dir", "seed{}".format(seed))
                ...
                if plot_step:
                    ...
                    ax.plot(plot_step, np.mean(
                        plot_p, axis=1), "-", c=c, label=run)
            box = ax.get_position()
    ...
    for ml in multiloops:
        ...
        for m in ml:
            ...
            if left_m[0] != "m":
                ...
                data["neighbor_left"].append(
                    abs(cg.connection_type(left_m, cg.connections(left_m))))
            ...
            if right_m[0] != "m":
                ...
                data["neighbor_right"].append(
                    abs(cg.connection_type(right_m, cg.connections(right_m))))
...
def generateParser():
    ...
    parser.add_argument("files", type=str, nargs="+",
                        help="One or more cg files that all have the same bulge graph!")
    parser.add_argument("--dssr-json", type=str, nargs="*",
                        help="One or more json files generated by x3dna-dssr. They have to be in the same order as the cg files.")
    parser.add_argument("-q", "--quiet", action="store_true",
                        help="Do not be so verbose!!!")
    parser.add_argument("-l", "--per-loop", action="store_true",
                        help="Print statistics per multiloop.")
    parser.add_argument("-i", "--interactive", action="store_true",
                        help="In combination with -l: Enter interactive mode for data analysis.")
    parser.add_argument("-m", "--method", type=str,
                        help="'CG' or 'Tyagi'. Method used for stacking detection in forgi.", default="Tyagi")
...
if __name__ == "__main__":
    ...
    if args.dssr_json and len(args.dssr_json) != len(args.files):
        parser.error(
            '--dssr-json must have the same number of arguments as files.')
    ...
    if args.dssr_json:
        ...
        for i, filename in enumerate(args.files):
            ...
            try:
                ...
                continue
                forgi, dssr = annot.compare_coaxial_stack_annotation(
                    args.method)
                both = forgi & dssr
                forgi = forgi - both
                dssr = dssr - both
                ...
                forgi_not_stacking += len(
                    list(f for f in dssr if f.forgi == "not stacking"))
                both_count += len(both)
                if not args.quiet:
                    print ("{} found by forgi, {} by dssr, {} by both".format(
                        len(forgi), len(dssr), len(both)))
                    for f in forgi:
                        print ("{} and {} stacking in forgi".format(
                            f.stems[0], f.stems[1]))
                    for d in dssr:
                        print ("{} and {} stacking in dssr (in forgi {})".format(
                            d.stems[0], d.stems[1], d.forgi))
                    for b in both:
                        print ("{} and {} stacking in both".format(
                            b.stems[0], b.stems[1]))
        print("======= SUMMARY ======")
        if args.dssr_json:
