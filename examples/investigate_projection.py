...
def get_parser():
    ...
    # Argument(s)
    parser.add_argument('cgfile', nargs="*",
                        help='One *.cg/*.coord files holding the RNA.')
    parser.add_argument('-w', '--width', type=int,
                        help='Width in Angstrom', default=200)
    parser.add_argument('-d', '--dpi', type=int,
                        help='Number of pixels in the image', default=30)
    return parser
...
class RnaDisplay(object):
    def __init__(self, args):
        ...
        mainframe.columnconfigure(0, minsize=min(
            int(args.width * 3.75 + 1), 100))
        mainframe.columnconfigure(1, minsize=min(
            int(args.width * 3.75 + 1), 100))
        mainframe.columnconfigure(2, minsize=min(
            int(args.width * 3.75 + 1), 100))
        mainframe.columnconfigure(3, minsize=min(
            int(args.width * 3.75 + 1), 100))
        ...
        self.zoom.set(10)
        # self.zoom.set(10)
        tkinter.ttk.Label(mainframe, text="Zoom:").grid(
        ...
        tkinter.ttk.Label(mainframe, text="Nucleotide:").grid(
        ...
        tkinter.ttk.Label(mainframe, text="offset x (Angstrom):").grid(
        ...
        tkinter.ttk.Label(mainframe, text="offset y (Angstrom):").grid(
            row=3, column=0, sticky="E")
        tk.Scale(mainframe, from_=-100, to_=100, orient=tk.HORIZONTAL, command=self.update,
                 variable=self.offset_y, length=180).grid(row=3, column=1, sticky="W")
        # Rotation
        self.inplane_rot = tk.IntVar()
        tkinter.ttk.Label(mainframe, text="rotate (degrees)").grid(
        ...
        if self.cg.project_from is not None:
            ...
            self.phi.set(int(p))
        tkinter.ttk.Label(mainframe, text="Projection angle: THETA").grid(
        ...
        tkinter.ttk.Label(mainframe, text="Projection angle: PHI").grid(
        ...
        tkinter.ttk.Label(mainframe, text="Image width in Angstrom").grid(
        ...
        tkinter.ttk.Label(mainframe, text="Image width in Pixels").grid(
    ...
    def saveImage(self):
        filename = asksaveasfilename(
            parent=self.mainframe, title='Save png image', defaultextension='.png')
        box = self.getBox()
