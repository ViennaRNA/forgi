...
def main():
    ...
    parser.add_option('-a', '--all', dest='all_entries_file', default=None,
                      help='Use a file containing the positions of each entry for this cg element.', type='str')
        ...
        for i in range(3):
            ...
            for conn_type in conn_types:
                ...
                if options.all_entries_file is not None:
                    ...
                    for c in coords:
                        pp.add_sphere(c, width=0.2, color_rgb=cmap(
                            i / float(stem_len)))

                    all_coords += coords
                    avg_coord = np.mean(coords, axis=0)
                    pp.add_sphere(avg_coord, width=0.6,
                                  color_rgb=cmap(i / float(stem_len)))
                else:
                    coords = ftua.avg_atom_poss[identifier]
                    pp.add_sphere(coords, width=0.2,
                                  color_rgb=cmap(i / float(stem_len)))

            avg_coord = np.mean(all_coords, axis=0)
            pp.add_sphere(avg_coord, width=0.9,
                          color_rgb=cmap(i / float(stem_len)))
