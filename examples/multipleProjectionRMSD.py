...
def get_parser():
    ...
    parser.add_argument('files', type=str, nargs='*',
                        help="An ORDERED list of *.coord files")
    parser.add_argument('--step-size', type=int,
                        help="How many steps two subsequent files are apart. Used for the x-axis of the plot.", default=1)
    parser.add_argument('--max-diff', type=str,
                        help="Maximal offset in FILES to be considered, or a ','-separated list of offsets to be considered.", default="20")
    parser.add_argument('--target-structure', type=str,
                        help="In addition, calculate the RMSD of every step to structure given here.")
    parser.add_argument('--hausdorff-reference', type=str,
                        help="In addition, calculate the Hausdorffdistance of every step to the png image file given here.")
    parser.add_argument('--hausdorff-scale', type=float,
                        help="Side length of the hausdorff-reference image in Angstrom.")
...
def get_refimg_longest_axis(ref_img):
    ...
    deg = math.degrees(math.atan2(
        best_points[1][0] - best_points[0][0], best_points[1][1] - best_points[0][1]))
    deg = 90 - deg
...
def main(args):
    ...
    for diff in diffs:
        ...
        for i in range(0, len(cgs) - diff):
            count += 1
            try:
                vrs1 = np.array([x for p in sorted(projs[i]._coords.keys())
                                 for x in projs[i]._coords[p]])
                vrs2 = np.array([x for p in sorted(
                    projs[i + diff]._coords.keys()) for x in projs[i + diff]._coords[p]])
    ...
    if args.target_structure:
        ...
        try:
            ...
            target_vrs = np.array([x for p in sorted(
                target_proj._coords.keys()) for x in target_proj._coords[p]])
            for proj in projs:
                try:
                    vrs1 = np.array(
                        [x for p in sorted(proj._coords.keys()) for x in proj._coords[p]])
        ...
        if target_proj_rmsds:
            axT.plot(xval, target_proj_rmsds,
                     label="Projection", color="green")
    ...
    if args.hausdorff_reference:
        ...
        local_optima = []
        for cg in cgs:
            score, img, params = fph.try_parameters(
                ref_img, args.hausdorff_scale, cg, degrees)
        ...
        axH.plot(xval, local_optima,
                 label="Local Minimization", color="lavender")
    ...
    ax.plot(p_rmsds[:, 0] * args.step_size, p_rmsds[:, 1],
            label="Projection", color="green", marker="o")
    ax.plot(cg_rmsds[:, 0] * args.step_size, cg_rmsds[:, 1],
            label="3D", color="blue", marker="o")
