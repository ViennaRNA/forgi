...
def parallel_localOpt(im_and_box, numFiles, filenames):
    ...
    try:
        ...
        for j in range(numFiles):
            cg = ftmc.CoarseGrainRNA(filenames[j])
            distance, img, _ = fph.locally_minimal_distance(
                ref_img, ref_box[1] - ref_box[0], cg)
            distances.append(distance)
        return distances
    except KeyboardInterrupt:
...
def get_parser():
    ...
    parser.add_argument('files', nargs="+",
                        help='One or more "*.cg"/"*.coord" files.')
    # Options
    ...
    parser.add_argument(
        '--scale', type=int, help='The width of the whole image in Angstrom. This is required if reference is a png image.')
    parser.add_argument('--outfile', type=str,
                        help='Currently only used if no reference is present. Write the resulting distance matrix to this file.')
    parser.add_argument('--show', action='store_true',
                        help='Show a figure. The figure does not show all calculated distances.')
    parser.add_argument('--cores', type=int, default=1,
                        help='Only used for many to many comparison. Number of cores for paralellization.')
    return parser
...
if __name__ == "__main__":
    ...
    if args.reference:
        if args.reference[-3:] == "png":
            ...
            if args.dpi:
                ref_img = scipy.misc.imresize(
                    ref_img, (args.dpi + 1, args.dpi + 1), "nearest")
            if args.scale:
                s = args.scale / 2
                ref_box = -s / 2, s / 2, -s / 2, s / 2
            else:
                parser.error(
                    "--scale is required if the reference is a png image")
            if args.show:
                fig, ax = plt.subplots()
            ...
            try:
                ref_proj = ftmp.Projection2D(
                    ref_cg, project_virtual_atoms=True)
            except ValueError:
            ...
            if args.dpi:
                ref_img, _ = ref_proj.rasterize(
                    args.dpi, bounding_square=ref_box)
            else:
                parser.error(
                    "If the reference is not a png image, --dpi is required")
        ...
        for f in args.files:
            ...
            if args.global_search or cg.project_from is None:
                distance, img, params = fph.globally_minimal_distance(
                    ref_img, args.scale, cg)
                fname = os.path.basename(f)
                print ("{}:\t{} distance (projected from {}, rotated by {}, offset {}. Globally optimized)".format(
                    fname, distance, params[0], params[1], params[2]))
            else:
                distance, img, params = fph.try_parameters(
                    ref_img, args.scale, cg)
                fname = os.path.basename(f)
                print ("{}:\t{} distance (projected from {}, rotated by {}, offset {}. Locally optimized)".format(
                    fname, distance, params[0], params[1], params[2]))
        if args.show:
        ...
        for i in range(numFiles):
            ...
            ref_img = (ref_img > np.zeros_like(ref_img))
            # if args.show:
            #    fig, ax=plt.subplots()
            #    ax.imshow(ref_img, interpolation="none", cmap='gray')
        ...
        if args.global_search:
            partial_calculation = partial(
                async_calculation, numFiles=numFiles, filenames=args.files)
        else:
            partial_calculation = partial(
                parallel_localOpt, numFiles=numFiles, filenames=args.files)
        pool = multiprocessing.Pool(args.cores)
        try:
            for i, line in enumerate(pool.imap(partial_calculation, toProcess)):
                sys.stderr.write(
                    '\rCalculation {}/{} done'.format(i + 1, numFiles))
                sys.stderr.flush()
                distances[i] = line
        ...
        print(distances)
        if args.outfile:
            np.savetxt(args.outfile, distances, delimiter=",", header=", ".join(
                os.path.basename(x) for x in args.files))
